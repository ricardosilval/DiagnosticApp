...
import cl.diagnosticapp.model.Log;
import cl.diagnosticapp.config.VersionConfig;
...
/**
 * Utilitario para registro de LOGS. Permite gestionar los logs del sistema y
 * guardarlos en base de datos si es necesario. Por defecto los logs son
 * guardados en la base de datos, de no ser requerido el constructor debe ser
 * iniciado con un boolean false
 *
 */
public class BaseLogger implements LogInterface {
    ...
    //private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(PortalLogger.class);
    ...
    public void info(Object message) {
        LogLocation caller = getCaller(Thread.currentThread().getStackTrace());
        log(INFO, message.toString(), null, caller.getClase(), caller.getMethod(), caller.getLine(), null);
    }
    ...
    private void log(int logLevel, String message, Throwable t, String callerClass, String callerMethod, int callerLine, String processId) {
        ...
        try {
            ...
            //cl.diagnosticapp.model.File stacktraceFile = new cl.diagnossticapp.File();
            ...
            logModel.setHostname(VersionConfig.getInstance().getHostName());
            logModel.setId(UUID.randomUUID().toString());
            logModel.setClase(callerClass);
            logModel.setLinea(callerLine);
            logModel.setMensaje(message);
            logModel.setProcesoId(processId);
            if (t != null) {
            ...
            dsS = (dsS == null || dsS.isEmpty()) ? "java:/DiagnosticAppDS" : dsS;
            ...
            preparedStatement.setString(1, logModel.getProcesoId()); //process_id
            preparedStatement.setString(2, new SimpleDateFormat("yyyy-MM-dd hh:mm:ss.SSS").format(logModel.getFecha())); //fecha
            preparedStatement.setInt(3, logModel.getTipo()); //type
            preparedStatement.setString(4, logModel.getHostname()); //hostname
            preparedStatement.setString(5, logModel.getMensaje()); //message
            ...
            preparedStatement.setInt(8, logModel.getLinea());//line
    ...
    public String cleanStacktrace(Throwable throwable) {
        ...
        for (String stline : stackTrace) {
            if (stline.toLowerCase().contains("cl.diagnosticapp") || stline.contains("Exception")) {
                filteredStackTrace.append(stline.replace("cl.diagnosticapp ->", "\t")).append("\n");
            }
        }
    ...
    private LogLocation getCaller(StackTraceElement[] stackTraceElements) {
        ...
        for (int i = 1; i < stackTraceElements.length; i++) {
            if (stackTraceElements[i].getClassName().equals("diagnosticapp.log.BaseLogger")) {
